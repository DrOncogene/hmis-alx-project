...
line interpreter for the HMIS console"""
import cmd
import readline
import sys
import json
...
class HMISCommand(cmd.Cmd):
    """class the defines the console object"""
    prompt = "(HMIS) "
    _classes = {"BaseModel": BaseModel, "Consult": Consult, "Drug": Drug, 
    ...
    def do_create(self, arg: str) -> None:
        ...
        if validate_args(args, 1) == -1:
            return
        if args[0] in HMISCommand._classes:
            """ print(args[0])
            id = int(len(storage.all()))
            print(id)
            if args[0] == Staff:
                new_obj = HMISCommand._classes[args[0]](id)
            else:"""
            new_obj = HMISCommand._classes[args[0]]()
        storage.new(new_obj)
        storage.save()
        print(new_obj.id)
...
def validate_args(args: list, n_args: int) -> int:
    if len(args) == 0:
        ...
        return -1
    if args[0] not in HMISCommand._classes:
